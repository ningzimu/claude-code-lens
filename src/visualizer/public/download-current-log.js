(function attachDownloadCurrentLog(root) {
  ...
  function buildMarkdownExport({
    ...
    lines.push(`**Requests in this export:** ${conversations.length}`);
    if (currentLogUrl) lines.push(`**Source log:** ${fileNameFromUrl(currentLogUrl) || currentLogUrl}`);
    lines.push('');
    lines.push('---');
    lines.push('');
    ...
    conversations.forEach((conv, index) => {
      ...
      lines.push('');
      lines.push(`**Time:** ${conv.started_at || conv.finished_at || 'Unknown'}`);
      lines.push(`**Agent:** ${conv.agentName || conv.agentRole || 'Unknown'}`);
      if (conv.agentId) lines.push(`**Agent ID:** ${conv.agentId}`);
      if (conv.agentConfidence) lines.push(`**Confidence:** ${conv.agentConfidence}`);
      if (conv.input?.model || conv.result?.data?.model) lines.push(`**Model:** ${conv.result?.data?.model || conv.input?.model}`);
      lines.push('');

      if (conv.input?.system) {
        lines.push('### System');
        lines.push('');
        lines.push(contentToMarkdown(conv.input.system, prompts));
        lines.push('');
      }
      ...
      if (Array.isArray(conv.input?.messages) && conv.input.messages.length) {
        ...
        lines.push('');
        conv.input.messages.forEach((message, messageIndex) => {
          lines.push(`#### ${message.role || 'message'} ${messageIndex + 1}`);
          lines.push('');
          lines.push(contentToMarkdown(message.content, prompts));
          lines.push('');
        });
      }
      ...
      if (Array.isArray(conv.result?.data?.content)) {
        lines.push(contentToMarkdown(conv.result.data.content, prompts));
      } else if (conv.result?.data?.text) {
        lines.push(markdownEscape(conv.result.data.text));
      } else if (conv.result?.data) {
        lines.push('```json');
        lines.push(JSON.stringify(conv.result.data, null, 2));
        lines.push('```');
      } else {
        lines.push('_No model output captured._');
      }
      lines.push('');
      lines.push('---');
      lines.push('');
